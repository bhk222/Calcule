...
const DiseaseCard: React.FC<{ disease: ProfessionalDisease }> = ({ disease }) => (
    <div className="bg-white rounded-lg shadow-md p-4 mb-3 border-l-4 border-blue-500">
        <h4 className="font-bold text-md text-blue-800">{disease.tableau} : {disease.name}</h4>
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="bg-blue-800 p-6 text-white text-center sticky top-0 z-10">
                <h2 className="text-2xl font-bold">Maladies Professionnelles</h2>
                <p className="text-blue-200 text-sm mt-1">Base de données pour experts (Législation Algérienne)</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 pt-0 space-y-2 bg-slate-100 custom-scrollbar">
               {filteredData.length > 0 ? (
                    filteredData.map((category: DiseaseCategory) => (
                        <details key={category.name} className="group" open>
                            <summary className="cursor-pointer p-3 bg-slate-200 rounded-md font-bold text-slate-800 group-open:bg-blue-200 group-open:text-blue-900 transition-colors flex justify-between items-center">
                                {category.name}
